      ...
      (async () => {
        try {
          ...
          const res = await fetch(
            `/api/books/borrow?bookId=${encodeURIComponent(String(book._id))}`
          );
          if (res.ok) {
            ...
            if (info.borrowed) {
              ...
              const isMine =
                ...
                session.user.email.toLowerCase() ===
                  (info.by?.userEmail || "").toLowerCase();
              if (!isMine) {
                toast.error(
                  `This book is currently borrowed by ${
                    info.by?.userName || info.by?.userEmail
                  }. Due: ${
                    info.by?.dueDate
                      ? new Date(info.by.dueDate).toLocaleDateString()
                      : "unknown"
                  }`
                );
                return; // do not open
